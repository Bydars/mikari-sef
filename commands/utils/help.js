  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (block(candidate).length > LIMIT) {
        await msg.temp(block(buffer), delay);
        buffer = line;
      } else {
        buffer = candidate;
      }
    }
    if (buffer.trim()) await msg.temp(block(buffer), delay);
  } catch (err) {
    console.error("❌ Error en sendChunked:", err);
    ...
    try {
      ...
      if (query && !query.startsWith("?")) {
        ...
        const category = categories.find(
          (c) => normalize(c) === normalize(query)
        );
        if (category) {
          const list = all
            .filter(
              (c) => normalize(c.category || "misc") === normalize(category)
            )
          ...
          if (!list.length) {
            return sendChunked(
              msg,
              `❌ No hay comandos en la categoría '${sanitize(category)}'.`
            );
          }
        ...
        return sendChunked(
          msg,
          `❌ No encontré comando o categoría \`${sanitize(query)}\`.`
        );
      }
      ...
      if (query.startsWith("?")) {
        ...
        if (!results.length) {
          return sendChunked(
            msg,
            `❌ No se encontraron comandos que coincidan con '${sanitize(term)}'.`
          );
        }
        ...
        const blocks = [
          `Resultados para '${sanitize(term)}' (prefix: \`${prefix}\`)`,
        ];
      ...
      console.error("❌ Error en comando help:", err);
      await msg.react("⚠️").catch(() => {});
    }
  },
};
